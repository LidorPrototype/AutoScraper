...
def download_pdf(source, hebsource, _from, _to):
    """
        Goal:
    ...
    while(continiue):
        ...
        for rpt in r.json()['Reports']:
            ...
            files=rpt['Files']
            temp=[item for item in rpt['Files'] if item['Type']==2]
            if len(temp)>0:
                ...
                url='https://mayafiles.tase.co.il/rpdf/'+rang1+'-'+rang2+'/P'+h+'-0'+str(ind)+'.pdf'
                print(url)
                res = requests.get(url)
                r = requests.get(url, stream=True)
                folder='{0}/'.format(source)
                with open(folder+companyId+'_'+pubDate+'.pdf', 'wb') as f:
                    f.write(r.content)
                    num_jsons+=1
