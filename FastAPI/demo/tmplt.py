...
class TmpltSpider(scrapy.Spider):
    ...
    try:
        ...
        if isinstance(prefixurl, np.floating):
            prefixurl = ""
    except IndexError:
        prefixurl = ""
    ...
    def parse_no_prefix(self, response):
        try:
            for key, value in self.namequery.items():
                tmp_lst = response.xpath(value).extract()
                for row in tmp_lst:
                    to_y = {key: row}
                    to_y.update(self.namemeta)
                    yield to_y
        except Exception as e:
            print(str(e))
